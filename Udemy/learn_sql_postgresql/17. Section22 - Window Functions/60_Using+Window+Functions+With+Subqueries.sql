/*
    We want to know when an order comes in that is 5 times greater than
    the customer's average order.
*/
...
/*
    Find any suppliers that had 3 times the normal quantity of orders over all
    their products versus the average order per month
*/
...
FROM (
        ...
        FROM (
                SELECT 
                    companyname,SUM(quantity) as total_orders,
